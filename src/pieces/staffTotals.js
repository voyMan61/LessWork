      ...
      <TableRow>
        {
          columnData.map(column => {
            return (<CustomTableCell key={column.id} numeric={column.numeric} padding={column.disablePadding
                ? 'none'
                : 'default'}>
              {column.label}
    ...
    return (<Paper>
      <Toolbar>
        <Typography style={{
            position: 'absolute',
            left: 60
          }} variant="title" id="tableTitle">Staff totals</Typography>
      </Toolbar>
        ...
        isLoading
          ...
          : (<div className={classes.tableWrapper}>
            <Table className={classes.table} aria-labelledby="tableTitle">
              <EnhancedTableHead numSelected={selected.length} order={order} orderBy={orderBy} onSelectAllClick={this.handleSelectAllClick} rowCount={hits.length}/>
              <TableBody>
                {
                  hits.map(n => {
                    return (<Tooltip placement="left" TransitionComponent={Zoom} title="View staff info">
                      <TableRow key={n.id} data-item={n} onClick={this.handleROpen.bind(this, n)}>
                        <CustomTableCell style={{
                            textAlign: 'center'
                          }} component="th" scope="row">
                          {n.name}</CustomTableCell>
                        <CustomTableCell style={{
                            textAlign: 'center'
                          }}>{n.offerings_taken}</CustomTableCell>
                        <CustomTableCell style={{
                            textAlign: 'center'
                          }}>{n.target}</CustomTableCell>
                        <CustomTableCell style={{
                            textAlign: 'center',
                            color: n.total_load < 0.9 * n.target
                              ? 'blue'
                              : n.total_load > 1.1 * n.target
                                ? 'red'
                                : 'green'
                          }}>{n.total_load}</CustomTableCell>
          ...
          ? (<Dialog open={this.state.open} scroll={this.state.scroll} aria-labelledby="scroll-dialog-title">
            ...
            <DialogTitle style={{
              ...
              <div style={{
                ...
                <Typography gutterBottom="gutterBottom" align="center" variant="display1">
                  {staffSelect.name}
                </Typography>
              </div>
